...
install_package() {
    ...
    case $OS in
        ubuntu|debian)
            PACKAGE_PATH="$TMPDIR/phase_cli_amd64_latest.deb"
        ...
        fedora|rhel|centos)
            PACKAGE_PATH="$TMPDIR/phase_cli_amd64_latest.rpm"
            HASH_PATH="$TMPDIR/hash"
    ...
    VERSION=$(phase -v 2>/dev/null || echo "Unknown")
