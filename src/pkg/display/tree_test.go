...
// The caller-supplied hint line renders below the context.
func TestRenderSecretsTreeEmptyShowsContext(t *testing.T) {
	out := captureStdout(t, func() {
		RenderSecretsTree(nil, false, "my-app", "Development", "💡 No secrets found at path /.\n")
	})

	for _, want := range []string{"my-app", "Development", "No secrets found at path /"} {
...
// With no hint supplied (no filters were applied), only the context renders.
func TestRenderSecretsTreeEmptyWithoutHint(t *testing.T) {
	out := captureStdout(t, func() {
	...
	if strings.Contains(out, "💡") {
		t.Fatalf("did not expect a hint line when none was supplied, got:\n%s", out)
	}
}
...
func TestRenderSecretsTreeHeaderUsesResolvedContext(t *testing.T) {
	...
	out := captureStdout(t, func() {
		RenderSecretsTree(secrets, false, "my-app", "Development", "")
	})
